...
namespace Brimborium.Latrans.JSON.Resolvers {
    public sealed class BuiltinResolver : IJsonFormatterResolver
    {
        ...
        {
            static Dictionary<Type, object> _formatterMap;
            static Dictionary<Type, object> getFormatterMap() => _formatterMap ??= new Dictionary<Type, object>()
            {
                // Primitive
            ...
            {
                object formatter;
                if (getFormatterMap().TryGetValue(t, out formatter))
                {
                    return formatter;
                }
