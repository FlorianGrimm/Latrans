...
namespace Brimborium.Latrans.JSON
{
    ...
    public static partial class JsonSerializerUtf8 {
        public static class NonGeneric
        {
            ...
            {
                if (value == null) {
                    return Serialize<object>(value, resolver);
                } else { 
            ...
            public static void Serialize(JsonWriter writer, object value, IJsonFormatterResolver resolver)
            {
                if (value == null)
                {
                    writer.WriteNull();
                ...
                {
                    Serialize(value.GetType(), writer, value, resolver);
                }
            }
            ...
            {
                GetOrAdd(type).serialize3.Invoke(writer, value, resolver);
            }
            ...
            {
                return GetOrAdd(type).deserialize4.Invoke(reader, resolver);
            }
            ...
            class CompiledMethods
            {
                ...
                public CompiledMethods(Type type)
                {
                    ...
                    {
                        var dm = new DynamicMethod("serialize3", null, new[] { typeof(JsonWriter), typeof(object), typeof(IJsonFormatterResolver) }, type.Module, true);
                        var il = dm.GetILGenerator();

                        il.EmitLdarg(0); // writer
                        il.EmitLdarg(1);
                        il.EmitUnboxOrCast(type);
                        il.EmitLdarg(2);
                        il.EmitCall(GetMethod(type, "Serialize", new[] { typeof(JsonWriter), null, typeof(IJsonFormatterResolver) }));
                        il.Emit(OpCodes.Ret);
                    ...
                    {
                        var dm = new DynamicMethod("Deserialize", typeof(object), new[] { typeof(JsonReader), typeof(IJsonFormatterResolver) }, type.Module, true);
                        var il = dm.GetILGenerator();

                        il.EmitLdarg(0); // reader
                        il.EmitLdarg(1);
                        il.EmitCall(GetMethod(type, "Deserialize", new[] { typeof(JsonReader), typeof(IJsonFormatterResolver) }));
                        il.EmitBoxOrDoNothing(type);
                        il.Emit(OpCodes.Ret);
