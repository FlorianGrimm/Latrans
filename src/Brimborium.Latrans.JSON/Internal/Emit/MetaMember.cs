...
namespace Brimborium.Latrans.JSON.Internal.Emit
{
    ...
    internal class InnerExceptionMetaMember : MetaMember
    {
        ...
        static readonly MethodInfo nongenericSerialize = ExpressionUtility.GetMethodInfo<JsonWriter>(writer => JsonSerializerUtf8.NonGeneric.Serialize(writer, default(object), default(IJsonFormatterResolver)));
        ...
        {
            // JsonSerializer.NonGeneric.Serialize(writer, value.InnerException, formatterResolver);
            argWriter.EmitLoad();
            argValue.EmitLoad();
            il.Emit(OpCodes.Callvirt, getInnerException);
