...
namespace Brimborium.Latrans.JSON.Formatters
{
    ...
    {
        delegate void SerializeMethod(object dynamicFormatter, JsonWriter writer, object value, IJsonFormatterResolver formatterResolver);
            ...
            if (!serializers.TryGetValue(type, out formatterAndDelegate))
            {
                lock (serializers)
                {
                    if (!serializers.TryGetValue(type, out formatterAndDelegate))
                    {
                        ...
                        var t = type;
                        {
                            var dm = new DynamicMethod("Serialize", null, new[] { typeof(object), typeof(JsonWriter), typeof(object), typeof(IJsonFormatterResolver) }, type.Module, true);
        ...
        {
            return PrimitiveObjectFormatter.Default.Deserialize(reader, formatterResolver);
        }
    }
}