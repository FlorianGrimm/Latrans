    ...
    internal static class RoslynExtensions {
        public static Compilation GetCompilationFromProject(IEnumerable<string> inputFiles, IEnumerable<string> inputDirectories, string[] preprocessorSymbols) {
            ...
            var references = new List<MetadataReference>
            {
                ...
                MetadataReference.CreateFromFile(typeof(Brimborium.Latrans.JSON.JsonReader).Assembly.Location),
            };
        ...
        public static AttributeData? FindAttribute(this IEnumerable<AttributeData> attributeDataList, string typeName) {
            return attributeDataList
                .Where(x => x.AttributeClass is object && x.AttributeClass.ToDisplayString() == typeName)
                .FirstOrDefault();
        }

        public static AttributeData? FindAttributeShortName(this IEnumerable<AttributeData> attributeDataList, string typeName) {
            return attributeDataList
                .Where(x => x.AttributeClass is object
                    && (x.AttributeClass.Name == typeName || x.AttributeClass.Name == typeName + "Attribute"))
                .FirstOrDefault();
        }
        ...
        public static object? GetSingleNamedArgumentValueFromSyntaxTree(this AttributeData attribute, string key) {
            ...
            if (syntaxNode is object) {
                foreach (var p in syntaxNode.DescendantNodes().OfType<AttributeArgumentSyntax>()) {
                    if (p.NameEquals is object && p.NameEquals.Name.Identifier.ValueText == key) {
                        if (p.Expression is LiteralExpressionSyntax les) {
        ...
        public static bool IsNullable(this INamedTypeSymbol symbol) {
            if (symbol.IsGenericType) {
                ...
                if (unboundGenericType.ToDisplayString() == "T?") {
                    return true;
                }
            }
            return false;
